...
class Add(Resource):
    def post(self):
        ...
        if money<=0:
            return jsonify(generateReturnDictionary(304, "The money amount entered must be > 0.00"))
...
class Transfer(Resource):
    def post(self):
        ...
        cash = cashWithUser(username)
        if cash<=0:
            return jsonify(generateReturnDictionary(304, "You're out of money, please take a look"))
        if not UserExist(to):
